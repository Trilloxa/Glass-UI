...
bool ImGui::Keybind(const char* label, int* key, int* mode) {
    ...
    ImRect mode_rect(window->DC.CursorPos + ImVec2(size - 10, 00), window->DC.CursorPos + ImVec2(size, 24));
    ImRect key_rect;
    ...
    if (*key != 0 && g.ActiveId != id) {
        strcpy_s(translation ? buf_display0 : buf_display1, active_key.c_str());
    }
    else if (g.ActiveId == id) {
        strcpy_s(translation ? buf_display0 : buf_display1, "-");
    }
    key_size = CalcTextSize(translation ? buf_display0 : buf_display1, NULL, true);
    ...
    window->DrawList->AddRectFilled(ImVec2(total_bb.Min - ImVec2(0, 5)), ImVec2(total_bb.Max + ImVec2(CalcItemWidth() - 208, -2)), GetColorU32(ImGuiCol_Header), style.FrameRounding);
    window->DrawList->AddRect(ImVec2(total_bb.Min - ImVec2(0, 5)), ImVec2(total_bb.Max + ImVec2(CalcItemWidth() - 208, -2)), ImColor(50 / 255.f + it_alpha->second, 50 / 255.f + it_alpha->second, 50 / 255.f + it_alpha->second), style.FrameRounding, ImDrawFlags_RoundCornersAll);


    window->DrawList->AddText(default_segu, 25.f, rect.Min + ImVec2(7, 0), ImColor(144, 144, 143, 255), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Text));

    RenderTextClipped(key_rect.Min - ImVec2(20, 1), key_rect.Max + ImVec2(CalcItemWidth() - 25, 2), translation ? buf_display0 : buf_display1, NULL, NULL, ImVec2(0.5f, 0.5f));
...
void ImGui::MultiCombo(const char* label, bool variable[], const char* labels[], int count)
{
    ...
    std::string preview = translation ? u8"Нет" : "None";
    ...
    if (ImGui::BeginCombo(label, preview.c_str(), 0)) // draw start
    {
        ...
        DrawBackgroundBlur(GetWindowDrawList(), g_pd3dDevice);
...
bool ImGui::TabButtonEx(const char* ico, const char* label, bool selected, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered || selected ? 0.5f : 0.0f, g.IO.DeltaTime * 8.f);
    it_nameanim->second *= style.Alpha;

    window->DrawList->AddRectFilled(bb.Min - ImVec2(0.000f, 5.000f), bb.Max + ImVec2(bb.Min.x - bb.Min.x + 76, bb.Min.y - bb.Min.y), ImColor(42 / 255.f, 44 / 255.f, 44 / 255.f, it_nameanim->second / 2), 6);

    window->DrawList->AddRect(bb.Min - ImVec2(0.000f, 5.000f), bb.Max + ImVec2(bb.Min.x - bb.Min.x + 76, bb.Min.y - bb.Min.y), ImColor(50 / 255.f, 50 / 255.f, 50 / 255.f, it_nameanim->second), 6);


    PushFont(ico_button);
    PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 0.3f + it_nameanim->second));
    PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(0.0f, 0.45f));
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(33, -8), bb.Max - style.FramePadding + ImVec2(30, 0), ico, NULL, &label_size, style.ButtonTextAlign, &bb);

    PopFont();

    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(70, -5), bb.Max - style.FramePadding + ImVec2(90, 0), label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopStyleVar();
    PopStyleColor();
...
bool ImGui::TabButton(const char* ico, const char* label, bool selected, const ImVec2& size_arg)
{
    return TabButtonEx(ico, label, selected, size_arg, ImGuiButtonFlags_None);
}



bool ImGui::Sub_TabButtonEx(const char* label, bool selected, bool rect, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f + 10, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (rect) window->DrawList->AddRectFilled(bb.Min + ImVec2(label_size.x + 17, 10), bb.Max + ImVec2(1, -5), ImColor(50 / 255.f, 50 / 255.f, 50 / 255.f, 255 / 255.f), 0);
...
bool ImGui::Sub_TabButton(const char* label, bool selected, bool rect, const ImVec2& size_arg)
{
    return Sub_TabButtonEx(label, selected, rect, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::OptButtonEx(const char* ico, const ImVec2& size_arg, const ImVec2& cursor_pos, bool rotation, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    SetCursorPos(ImVec2(cursor_pos));
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered ? 1.f : 0.f, g.IO.DeltaTime * 9.f);
    it_nameanim->second *= style.Alpha;

    if (rotation) ImRotateStart();

    ImGui::PushFont(ico_bold);

    ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(134, 134, 133, 255));
...
bool ImGui::OptButton(const char* ico, const ImVec2& size_arg, const ImVec2& cursor_pos, bool rotation)
{
    return OptButtonEx(ico, size_arg, cursor_pos, rotation, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = CalcItemWidth();
    const float distance = w + 16;

    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(w , 0) + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_alpha->second = ImLerp(it_alpha->second, hovered || *v ? 0.1f : 0.f, g.IO.DeltaTime * 8.f);
    ...
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1 + distance, 1), check_bb.Max + ImVec2(14 + distance, -7), ImColor(65 / 255.f, 66 / 255.f, 67 / 255.f, 1.00f), 10);

    window->DrawList->AddRect(check_bb.Min + ImVec2(1 + distance, 1), check_bb.Max + ImVec2(14 + distance, -7), ImColor(65 / 255.f + it_alpha->second, 66 / 255.f + it_alpha->second, 67 / 255.f + it_alpha->second, it_alpha->second * 10), 10);


    window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + distance + 13 + it_nameanim->second * 20, check_bb.Min.y + 12.6f), 8.f, ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 0.2f + it_nameanim->second), 24);

    PushStyleColor(ImGuiCol_Text, ImVec4(144 / 255.f, 144 / 255.f, 145 / 255.f, 1.f));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 115, label_size.y + style.FramePadding.y * 2.0f));
    ...
    it_alpha->second = ImLerp(it_alpha->second, popup_open || IsItemClicked() || hovered ? 0.2f : 0.f, g.IO.DeltaTime * 6.f);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min - ImVec2(0, 5), ImVec2(value_x2 - 22, bb.Max.y + 3), GetColorU32(ImGuiCol_Header), style.FrameRounding);
        window->DrawList->AddRect(bb.Min - ImVec2(0, 5), ImVec2(value_x2 - 22, bb.Max.y + 3), ImColor(50 / 255.f + it_alpha->second, 50 / 255.f + it_alpha->second, 50 / 255.f + it_alpha->second), style.FrameRounding, ImDrawFlags_RoundCornersAll);
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(144 / 255.f, 144 / 255.f, 145 / 255.f, 1.f));
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(3, -2), ImVec2(value_x2 + 50, bb.Max.y), preview_value, NULL, NULL);
    ...
    ImRotateStart();
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(w + 28, -8), ImVec2(value_x2 + 50, bb.Max.y), label, NULL, NULL);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    {
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x, w - 53);
    }
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w - 53, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(0, 9));
        }

    // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x + 4, 9));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::SetCursorPosX(w - 9);

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(95, label_size.y + style.FramePadding.y * 1.9f));
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 13), ImVec2(total_bb.Min.x + 95, total_bb.Min.y + 16), ImColor(88, 87, 86, 255), 10);

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y + 13), ImVec2(frame_bb.Min.x + it_filing->second.value + 8, total_bb.Min.y + 16), ImColor(88, 86, 85, 255), 10);
    ...
    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_filing->second.value + 5, grab_bb.Min.y + 12.3f), it_namemark->second, ImColor(255, 255, 255, 255), 24);
    ...
    RenderTextClipped(ImVec2(total_bb.Min.x - w + 63, total_bb.Min.y - 30), ImVec2(total_bb.Min.x + 2, total_bb.Min.y + 56), value_buf, value_buf_end, NULL, ImVec2(0.9f, 0.5f));

    PushStyleColor(ImGuiCol_Text, ImVec4(144 / 255.f, 144 / 255.f, 145 / 255.f, 1.f));

    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - w - 60, frame_bb.Min.y + style.FramePadding.y - 3.0f), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 0, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 6.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f) + style.FramePadding.x, frame_size.y);
    ...
    if (!is_multiline)
    {
        ...
        it_alpha->second = ImLerp(it_alpha->second, IsItemActive() || IsItemClicked() || hovered ? 0.2f : 0.f, g.IO.DeltaTime * 6.f);

        it_alpha->second *= style.Alpha;

        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_Header), true, style.FrameRounding);

        window->DrawList->AddRect(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), ImColor(50 / 255.f + it_alpha->second, 50 / 255.f + it_alpha->second, 50 / 255.f + it_alpha->second, 1.0f), 6);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect)) draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(-2, +3.0f), rect.Max + ImVec2(3, 7), GetColorU32(ImGuiCol_TextSelectedBg), 5);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(0, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_filing->second.value + 0, 13), frame_bb.Min + ImVec2(it_filing->second.value + 1, 30), GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(0, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    PushFont(ico);
    if (label_size.x > 0) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - CalcItemWidth() + 10, frame_bb.Min.y + style.FramePadding.y + 7), label);
    PopFont();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    it_nameanimX->second = ImLerp(it_nameanimX->second, it_nameanimX->second ? square_sz * 8 - 7 : 0.f, g.IO.DeltaTime * 6.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_ItemInnerSpacing, ImVec2(7, 7));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        RenderText(ImVec2(bb_inner.Max.x - 37, bb_inner.Min.y + 0), "P");
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_hov->second = ImLerp(it_hov->second, selected || hovered ? 1.0f : 0.f, g.IO.DeltaTime * 13.f);
    it_hov->second *= style.Alpha;

    if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
        hovered = true;

        PushStyleColor(ImGuiCol_Text, ImVec4(1.00f, 1.00f, 1.00f, 1.00f - it_hov->second / 2));
        RenderTextClipped(text_min + ImVec2(it_hov->second * 13, -2), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
        PopStyleColor();

        window->DrawList->AddCircleFilled(ImVec2(text_min.x + 4, text_max.y - 12), 3, ImColor(1.00f, 1.00f, 1.00f, it_hov->second * 2), 24);
